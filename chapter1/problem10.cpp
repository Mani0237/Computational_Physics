...
int main(){
    ...
    cin >> alpha ;   
    x_exact = (1/alpha)*log(1 + alpha*vx0*t) ;
    y_exact = (vy0 + g/(2*alpha*vx0))*log(1 + alpha*vx0*t)/(alpha*vx0) - 0.25*g*t*t - g*t/(2*alpha*vx0) ;
    ofstream myfile;
    myfile.open("problem10.txt");
    myfile << t << '\t' << x << '\t' << y << '\t' << x_exact << '\t' << y_exact << endl;
